...
(**最後に赤色に点滅**したときは、クラウドへの通知に失敗しています。何度試しても症状が変わらないときは、IoT Centralの設定、Wi-Fi パスワードの設定などを再度ReButtonに設定してください。）
...
①`Sign in to Azure`をクリックして、自分のAzureアカウントを使ってサインインします。  
...
最初に配置(デプロイ)が終了した`Function App`の①`自分が作成したFunction name`を右クリックし、②`Copy Function Url`をクリックしてURLをコピーしておきます。

![82](img/82.png)

3章でやったのと同じように、`Azure IoT Central`上で、左の①`デバイステンプレート`をクリックして、②`ReButton`デバイステンプレートをクリックしてください。
...
最後に、`コールバックURL`に、先にAzure Function Appでコピーした①`(自分のFunction URL)`を貼り付けし、②`保存`をクリックして、ルールの追加が終了です。
...
VS Codeにて、最初に配置(デプロイ)が終了した`Function App`の①`自分が作成したFunction name`を右クリックし、②`Start Streaming Logs`をクリックしてストリームログを開始しておきます。
...
|この節での実施概要|
|:--|
|今後自分のAzure IoT Centralアカウントに課金されないように、自分のAzure IoT Centralサービス上に作成したアプリケーションを消去します。|

Azure IoT Centralの画面で、①`管理`をクリックし、②`アプリケーションの設定`をクリックします。  
...
|この節での実施概要|
|:--|
|今後自分のAzureアカウントに課金されないように、自分のAzureモニターサービスに作成したアクショングループを消去します。|
...
|この節での実施概要|
|:--|
|今後自分のAzureアカウントに課金されないように、自分のAzureサービスに作成したリソースグループを消去します。|
